...
if __name__ == "__main__":
    ...
    while True:
        ...
        if player.capital > 0:
            for player in game.players:
                bet_amount_str = input(
                    f"Enter an integer bet amount less than or equal to {player.capital} for {player.name}: "
                )
        ...
        # play again?
        do_play_again = str.lower(
            input("Enter Y/y to play again, any other key to stop: ")
        )
        if do_play_again != "y":
            break
